...
SYSTEM_DEVICE=$(cat /fstab.* /etc/fstab | grep -v "#" | grep -m 1 " / \| /system \| /system_root " | tr -s " ");
if ($SLOT_USED); then
  if (! `echo $SYSTEM_DEVICE | grep slotselect >/dev/null 2>&1`); then
    if (! `echo $SYSTEM_DEVICE | grep "$SLOT_SUFFIX" >/dev/null 2>&1`); then
      SLOT_USED=false
    fi
  fi
fi
for i in $SYSTEM_DEVICE; do
  if (! `echo $SYSTEM_DEVICE | grep "$SLOT_SUFFIX" >/dev/null 2>&1`); then
    SYSTEM_DEVICE=$i$SLOT_SUFFIX
  else
    SYSTEM_DEVICE=$i
  fi
  break
...
#    rm -rf /data/security/*
  ...
  if ($PATCHBOOTIMAGE); then
    ...
    if ($CONTINUE); then
      ...
      if ($CONTINUE); then
        if ($PERMISSIVE); then
          # once for default SuperSU rules, once to make everything fake-permissive, both are needed
          LD_LIBRARY_PATH=$SYSTEMLIB /su/bin/supolicy --file /sutmp/sepolicy /sutmp/sepolicy.tmp --sdk=$API
          LD_LIBRARY_PATH=$SYSTEMLIB /su/bin/supolicy --file /sutmp/sepolicy.tmp /sutmp/sepolicy.patched "permissive *"
        else
          LD_LIBRARY_PATH=$SYSTEMLIB /su/bin/supolicy --file /sutmp/sepolicy /sutmp/sepolicy.patched --sdk=$API
        fi
